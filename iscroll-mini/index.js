/**
 * @fileoverview Iscroll.
 * @author basilwang wanghuajie@yunwii.com
 */
/**
 * KISSY.use('../index',function(S,Iscroll){
 *		Iscroll.init();
 * });
 */
KISSY.add("h5-home/widgets/iscroll/index", function() {
	(function(window, document, Math) {
		...
		var utils = function() {
			...
			me.tap = function(e, eventName) {
				var p = e.changedTouches ? e.changedTouches[0] : e, target = e.target, ev;
				ev = document.createEvent("Event");
				ev.initEvent(eventName, true, true);
				ev.pageX = p.pageX;
				ev.pageY = p.pageY;
				ev.screenX = p.screenX;
				ev.screenY = p.screenY;
				ev.clientX = p.clientX;
				ev.clientY = p.clientY;
			...
			me.click = function(e) {
				...
				if (!/(SELECT|INPUT|TEXTAREA)/i.test(target.tagName)) {
					...
					ev.initMouseEvent("click", true, true, e.view, 1, p.screenX, p.screenY, p.clientX, p.clientY, e.ctrlKey, e.altKey, e.shiftKey, e.metaKey, 0, null);
					ev.pageX = p.pageX;
					ev.pageY = p.pageY;
		...
		IScroll.prototype = {
			...
			_end: function(e) {
				...
				if (newX != this.x || newY != this.y) {
					...
					this.scrollTo(newX, newY, time, easing, this.options.momentum && this.options.probeType == 4);
					return;
				}
				this._execEvent("scrollEnd");
			},
			...
			scrollTo: function(x, y, time, easing, animate) {
				var that = this;
				easing = easing || utils.ease.circular;
				this.isInTransition = !animate && this.options.useTransition && time > 0;
				if (!animate && (!time || this.options.useTransition && easing.style)) {
	...
	window.FixClickThrough && window.FixClickThrough.bind("click", function(e) {
		return !!e._constructed;
	});
	return window.iScroll = window.IScroll;
}, {
	requires: [ "touch" ]
});
